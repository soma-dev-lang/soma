...
pub static BUILTINS: &[BuiltinDoc] = &[
    ...
    doc("format", "string", "format(fmt: String, args...) -> String",
        "printf subset: %d %s %f %.2f %8.2f %e %.3e %3d %-8s %05d %% — %e is C-style scientific (6.022000e+23); widths, precision (rounded half away from zero on the decimal text), left-align with '-', zero-pad with '0'."),
    ...
    doc("substring", "string", "substring(s: String, start: Int, end: Int) -> String",
        "Character-based slice [start, end) — both indexes are clamped to 0..len(s), end exclusive. A negative end gives an empty string; use slice() for indexes relative to the end."),
    ...
    doc("asin", "math", "asin(x: Float) -> Float", "Arc sine (x in [-1, 1], else kind range)."),
    doc("acos", "math", "acos(x: Float) -> Float", "Arc cosine (x in [-1, 1], else kind range)."),
    doc("pi", "math", "pi() -> Float", "The constant π."),
    ...
    doc("pow", "math", "pow(base: Int|Float, exp: Int|Float) -> Float",
        "base raised to exp (always a Float; `ipow` for an exact Int power)."),
    doc("ipow", "math", "ipow(base: Int, exp: Int) -> Int",
        "Exact Int power (exp ≥ 0; kind range past the Int size limit). `to_int(pow(3, 40))` is off by 33, `ipow(3, 40)` is exact."),
    doc("min", "math", "min(a, b) -> Int|Float | min(list: List) -> Int|Float",
        "Smaller of two numbers, or the minimum of a list (Float if any element is). An empty list gives () (no minimum exists)."),
    doc("max", "math", "max(a, b) -> Int|Float | max(list: List) -> Int|Float",
        "Larger of two numbers, or the maximum of a list (Float if any element is). An empty list gives () (no maximum exists)."),
    ...
    doc("avg", "math", "avg(list: List) -> Int|Float",
        "Mean of a list of numbers: finite operands are averaged exactly before Float rounding, avoiding intermediate overflow and cancellation. An exact mean of Ints stays an Int; () when empty."),
    doc("parse_int", "math", "parse_int(s: String, base: Int?) -> Int | ()",
        "Strict integer parse: () unless the WHOLE string is an integer (\"1.5\", \"12abc\", \"\" → ()). parse_int(\"ff\", 16) = 255 (base 2..36, no 0x prefix). to_int() is lenient and truncates."),
    ...
    doc("clamp", "math", "clamp(v, lo, hi) -> Int|Float",
        "Constrain numeric v to [lo, hi] using exact Int/Float comparisons; return the selected operand with its type. Reject nonnumeric operands, reversed bounds and NaN bounds; a NaN value stays NaN."),
    ...
    doc("shr", "math", "shr(a: Int, n: Int) -> Int",
        "Arithmetic shift right by a nonnegative Int count; arbitrarily large counts give 0 (nonnegative a) or -1 (negative a)."),
    ...
    doc("bit_len", "math", "bit_len(a: Int) -> Int",
        "Exact number of significant bits in the magnitude, including BigInt."),

    // ── collection ──────────────────────────────────────────────────
    doc("list", "collection", "list(items...) -> List",
        "Build a list of the arguments; list(list(1, 2), 3) is [[1, 2], 3]. Use push(xs, x) to append."),
    ...
    doc("range", "collection", "range(start: Int, end: Int, step?: Int) -> List<Int>",
        "Integers from start toward end (exclusive); bounds and step are 64-bit Ints. A negative step counts down; zero raises kind range. Direct for-loops do not materialize the list, including stepped ranges."),
    ...
    doc("top", "pipeline", "top(rows: List, n: Int) -> List",
        "First n elements, capped at the list length. n must be a nonnegative Int; negative raises kind range, wrong types raise kind type."),
    doc("bottom", "pipeline", "bottom(rows: List, n: Int) -> List",
        "Last n elements, capped at the list length. n must be a nonnegative Int; negative raises kind range, wrong types raise kind type."),
    ...
    doc("median", "math", "median(xs: List) -> Int | Float",
        "Order Int/Float values exactly and preserve the middle value (mean of the two middles for even n). An Int middle stays exact; any NaN input makes the result NaN."),
    doc("pstdev", "math", "pstdev(xs: List) -> Float",
        "Population standard deviation (divide by n). Preserve finite and subnormal deviations even when their variance is outside Float range."),
    doc("stddev", "math", "stddev(xs: List) -> Float", "Same as pstdev (population)."),
    doc("stdev", "math", "stdev(xs: List) -> Float",
        "SAMPLE standard deviation (divide by n - 1); needs two values. Preserve finite and subnormal deviations even when their variance is outside Float range."),
    ...
    doc("chr", "string", "chr(n: Int) -> String", "The character with code point n: chr(65) == \"A\". Raises kind range outside 0..0x10FFFF or for surrogate code points."),
