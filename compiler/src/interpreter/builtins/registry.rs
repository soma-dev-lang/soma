...
pub static BUILTINS: &[BuiltinDoc] = &[
    ...
    doc("refusal", "http", "refusal(kind: String, detail?: String) -> Map",
        "The body and HTTP status a RAISED error of that kind would produce ({error, kind, detail}, e.g. `conflict` → 409, `not_found` → 404, your own tag → 400). Return it when the handler must refuse AND keep what it wrote (an audit entry): raising rolls the writes back, returning does not."),
