...
pub static BUILTINS: &[BuiltinDoc] = &[
    ...
    doc("sum", "math", "sum(list: List) -> Int|Float",
        "Sum of a list of numbers (Int-exact unless any element is a Float); 0 when empty."),
    ...
    doc("shl", "math", "shl(a: Int, n: Int) -> Int",
        "64-bit left shift: bits shifted past bit 63 are dropped (a bit operation, like bxor — xorshift-friendly); n ≥ 64 gives 0. For an exact 2^n multiple use a * pow(2, n)."),
    ...
    doc("stddev", "math", "stddev(xs: List) -> Float", "Same as pstdev."),
    doc("stdev", "math", "stdev(xs: List) -> Float", "Same as pstdev."),
    doc("variance", "math", "variance(xs: List) -> Float",
        "Population variance (divide by n) — statistics.pvariance."),
