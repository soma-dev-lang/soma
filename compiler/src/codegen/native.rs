...
pub fn generate_native_source_with_config(
    ...
    // Save the classifier's PER-HANDLER mode decisions before we
    // force everything to Rug-as-primary. If every handler classified
    // as Direct, the cell is "pure direct" — no operation can need
    // BigInt, no panic-on-overflow can trigger. native_ffi.rs reads
    // a marker from the generated source and sets OC=false in the
    // Cargo profile, giving Direct cells unchecked-arithmetic speed.
    let all_direct_originally = modes.iter().all(|m| *m == Mode::Direct);
    ...
    out.push_str("#![allow(unused_parens, unused_mut, unused_variables, unused_assignments, dead_code, unused_imports, non_snake_case, arithmetic_overflow, unconditional_panic, clippy::all)]\n");
    out.push_str("// Do not edit — regenerate from .cell source\n\n");
    out.push_str(r#"
#[inline] fn _soma_cmp_if(a: i64, b: f64) -> Option<std::cmp::Ordering> {
    use std::cmp::Ordering::*;
    if b.is_nan() { return None; }
    if b >= 9223372036854775808.0 { return Some(Less); }
    if b < -9223372036854775808.0 { return Some(Greater); }
    Some(a.cmp(&(b as i64)).then_with(|| 0.0f64.partial_cmp(&b.fract()).unwrap()))
}
"#);
    // recursion depth guard: a deep native recursion overflowed the thread
    // stack and ABORTED the whole process (soma serve died on one request);
    // past the limit it is the interpreter's stack_overflow error instead
    ...
    if uses_random {
        ...
        out.push_str("#[inline(always)]\nfn _soma_random_below(lo: i64, hi: i64) -> i64 { if hi <= lo { return lo; } let width = (hi as i128 - lo as i128) as u128; (lo as i128 + (((_soma_random_u64() as u128) * width) >> 64) as i128) as i64 }\n\n");
    }
...
fn gen_direct_inner_fn(
    ...
    out.push_str(&format!(
        "fn inner_{}({}) -> {} {{\n    let _depth_guard = _SomaDepth::enter();\n",
...
struct FnGenerator {
    ...
    /// Variables that hold a fixed-size i64 buffer (`Vec<i64>`),
    /// created via the `buffer(N)` builtin and accessed via
    /// `buf_get(b, i)` and `buf_set(b, i, v)`. The codegen emits
    /// these as direct Vec indexing, providing O(1) random access
    ...
    temporary_counter: RefCell<usize>,
...
impl FnGenerator {
    ...
    /// A generated binding that cannot shadow a user variable or another
    /// generated loop counter, including inside nested loops.
    fn fresh_local(&self) -> String {
        loop {
            let mut next = self.temporary_counter.borrow_mut();
            let name = format!("_soma_tmp_{}", *next);
            *next += 1;
            if !self.var_types.contains_key(&name) { return name; }
        }
    }
    ...
    fn infer_stmt_types(&mut self, stmt: &Statement) {
        match stmt {
            Statement::Let { name, value } => {
                // Detect `let X = buffer(N)` — mark X as a Vec<i64>
                // buffer var. Doesn't go in var_types as a NativeType
                // (it's not Int/Float/Bool/String), tracked separately
                // in buf_vars.
                if let Expr::FnCall { name: fname, .. } = &value.node {
                    if fname == "buffer" {
    ...
    fn gen_stmt_direct(&self, stmt: &Statement, indent: usize) -> String {
        ...
        match stmt {
            Statement::Let { name, value } => {
                // Buffer creation: `let X = buffer(N)` →
            ...
            Statement::While { condition, body, bound } => {
                ...
                if bound.is_some() { s.push_str(&format!("{}let mut {counter}: u64 = 0;\n", ind)); }
                s.push_str(&format!("{}while {} {{\n", ind, cond));
                if let Some(b) = bound { s.push_str(&format!("{}    {counter} += 1; if {counter} > {}u64 {{ panic!(\"soma:loop_bound: [loop_bound({})] exceeded — the declared bound is part of the cost/termination proof\") }}\n", ind, b, b)); }
                for st in body {
                    s.push_str(&self.gen_stmt_direct(&st.node, indent + 1));
                }
                s.push_str(&format!("{}}}\n", ind));
    ...
    fn gen_expr_direct(&self, expr: &Expr, target_ty: NativeType) -> String {
        match expr {
            ...
            Expr::CmpOp { left, op, right } => {
                let lt = self.infer_expr_type(&left.node);
                let rt = self.infer_expr_type(&right.node);
                if matches!((lt, rt), (NativeType::Int, NativeType::Float) | (NativeType::Float, NativeType::Int)) {
                    return self.gen_mixed_comparison(&left.node, *op, &right.node, lt);
                }
            ...
            Expr::FnCall { name, args } => {
                let out = self.gen_fn_call_direct(name, args, target_ty);
                // an Int-valued call (floor, len, a sibling) in a Float
                // context: not every emitter coerces to its target, and
                // `f64 + i64` is a rustc error. `as f64` is idempotent.
                if target_ty == NativeType::Float && self.infer_expr_type(expr) == NativeType::Int {
                    self.coerce_direct(out, NativeType::Int, NativeType::Float)
                } else {
                    out
                }
            }
            other => {
                self.err(format!("unsupported expression in a [native] handler: {} — not part of the native vocabulary (see the Performance section of llms.txt)", crate::ast::render_expr(other)));
                "0i64".to_string()
            }
        }
    }

    /// Compare Int/Float without converting the Int to a rounded float.
    fn gen_mixed_comparison(&self, left: &Expr, op: CmpOp, right: &Expr, left_ty: NativeType) -> String {
        let operand = |e: &Expr, ty: NativeType| {
            if ty == NativeType::Int && self.mode == Mode::Rug { self.gen_expr_rug(e) }
            else { self.gen_expr_direct(e, ty) }
        };
        let right_ty = if left_ty == NativeType::Int { NativeType::Float } else { NativeType::Int };
        let (l, r) = (operand(left, left_ty), operand(right, right_ty));
        let comparison = if self.mode == Mode::Rug {
            if left_ty == NativeType::Int { "l.partial_cmp(&r)" } else { "r.partial_cmp(&l).map(std::cmp::Ordering::reverse)" }
        } else if left_ty == NativeType::Int { "_soma_cmp_if(l, r)" }
        else { "_soma_cmp_if(r, l).map(std::cmp::Ordering::reverse)" };
        let test = match op {
            CmpOp::Eq => "o == Some(std::cmp::Ordering::Equal)",
            CmpOp::Ne => "o != Some(std::cmp::Ordering::Equal)",
            CmpOp::Lt => "o == Some(std::cmp::Ordering::Less)",
            CmpOp::Gt => "o == Some(std::cmp::Ordering::Greater)",
            CmpOp::Le => "matches!(o, Some(std::cmp::Ordering::Less | std::cmp::Ordering::Equal))",
            CmpOp::Ge => "matches!(o, Some(std::cmp::Ordering::Greater | std::cmp::Ordering::Equal))",
        };
        // Evaluate both operands before introducing temporary bindings: the
        // user's right operand may itself reference a local named `l`.
        format!("{{ let (l, r) = ({}, {}); let o = {}; {} }}", l, r, comparison, test)
    }
    ...
    fn gen_fn_call_direct(&self, name: &str, args: &[Spanned<Expr>], target_ty: NativeType) -> String {
        match name {
            // Buffer accessors — emit raw Vec indexing.
            "buf_get" if args.len() == 2 => {
                if let Expr::Ident(buf_name) = &args[0].node {
                    let i = self.gen_expr_direct(&args[1].node, NativeType::Int);
                    let inner = format!("{}[({}) as usize]", buf_name, i);
                    return self.coerce_direct(inner, NativeType::Int, target_ty);
                }
                self.err("buf_get: first arg must be an identifier");
                "0i64".to_string()
            }
            "buf_set" if args.len() == 3 => {
                // buf_set is statement-like — when it appears as an
                // expression, we wrap it in a block that returns 0i64
                // so the type system stays happy. The ExprStmt path
                // detects the call directly and emits a clean
                // assignment.
                if let Expr::Ident(buf_name) = &args[0].node {
                    let i = self.gen_expr_direct(&args[1].node, NativeType::Int);
                    let v = self.gen_expr_direct(&args[2].node, NativeType::Int);
                    return format!("{{ {}[({}) as usize] = {}; 0i64 }}", buf_name, i, v);
                }
    ...
    fn gen_stmt_rug(
        ...
        match stmt {
            ...
            Statement::For { var, iter, body, bound } => {
                ...
                let iter_code = self.gen_for_iter_direct(&iter.node);
                let counter = self.fresh_local();
                let iteration_var = self.fresh_local();
                let small = self.small_int_vars.contains(var)
                    || self.var_types.get(var).copied().unwrap_or(NativeType::Int) != NativeType::Int;
                let pre = if bound.is_some() { format!("{}let mut {counter}: u64 = 0;\n", ind) } else { String::new() };
                let guard = match bound { Some(b) => format!("{}    {counter} += 1; if {counter} > {}u64 {{ panic!(\"soma:loop_bound: [loop_bound({})] exceeded — the declared bound is part of the cost/termination proof\") }}\n", ind, b, b), None => String::new() };
                if small {
                    ...
                    let mut s = format!("{}{}for {iteration_var} in {} {{\n{}{}    let mut {}: Integer = Integer::from({iteration_var});\n", pre, ind, iter_code, guard, ind, var);
                    s.push_str(&self.gen_body_rug(body, indent + 1, ctx));
                    s.push_str(&format!("{}}}\n", ind));
                    s
                }
            ...
            Statement::ExprStmt { expr } => {
                ...
                if let Expr::FnCall { name: fname, args } = &expr.node {
                    if fname == "buf_set" && args.len() == 3 {
                        if let Expr::Ident(buf_name) = &args[0].node {
                            let i = self.gen_expr_direct(&args[1].node, NativeType::Int);
                            // BigInt arithmetic for the value (this body is the
                            // retry after an i64 overflow): a Buf holds 64-bit
                            // Ints, so a value that does not fit is a `range`
                            // error, not a second overflow panic
                            let v = self.gen_expr_rug(&args[2].node);
                            return format!(
                                "{}{{ let _v: Integer = {}; match _v.to_i64() {{ Some(_n) => {{ {}[({}) as usize] = _n; }}, None => panic!(\"soma:range: a Buf holds 64-bit Ints — {{}} does not fit (keep it in a scalar, which promotes to BigInt, or mask it)\", _v) }} }}\n",
                                ind, v, buf_name, i
                            );
                        }
                    }
    ...
    fn gen_expr_rug(&self, expr: &Expr) -> String {
        match expr {
            ...
            other => {
                self.err(format!("unsupported expression in a [native] handler (BigInt mode): {}", crate::ast::render_expr(other)));
                "Integer::from(0i64)".to_string()
            }
        }
    }
        ...
        match expr {
            Expr::CmpOp { left, op, right } => {
                ...
                if lt == NativeType::Int && rt == NativeType::Int {
                    ...
                    format!("{} {} {}", l, op_str, r)
                } else if matches!((lt, rt), (NativeType::Int, NativeType::Float) | (NativeType::Float, NativeType::Int)) {
                    self.gen_mixed_comparison(&left.node, *op, &right.node, lt)
                } else if lt == NativeType::Bool && rt == NativeType::Bool {
    ...
    fn gen_fn_call_rug(&self, name: &str, args: &[Spanned<Expr>]) -> String {
        match name {
            // Buffer accessors — emit Vec indexing wrapped in Integer
            // (callers in Rug mode expect Integer return values).
            "buf_get" if args.len() == 2 => {
                if let Expr::Ident(buf_name) = &args[0].node {
                    let i = self.gen_expr_direct(&args[1].node, NativeType::Int);
                    return format!("Integer::from({}[({}) as usize])", buf_name, i);
                }
                self.err("buf_get: first arg must be an identifier");
                "Integer::from(0i64)".to_string()
            }
            "buf_set" if args.len() == 3 => {
                if let Expr::Ident(buf_name) = &args[0].node {
                    let i = self.gen_expr_direct(&args[1].node, NativeType::Int);
                    // the value in BigInt arithmetic (this is the retry after
                    // an i64 overflow): a Buf holds 64-bit Ints, so a value
                    // that does not fit is a clear `range` error, not a
                    // second overflow panic
                    let v = self.gen_expr_rug(&args[2].node);
                    return format!(
                        "{{ let _v: Integer = {}; match _v.to_i64() {{ Some(_n) => {{ {}[({}) as usize] = _n; }}, None => panic!(\"soma:range: a Buf holds 64-bit Ints — {{}} does not fit (keep it in a scalar, which promotes to BigInt, or mask it)\", _v) }} Integer::from(0i64) }}",
                        v, buf_name, i
                    );
                }
                self.err("buf_set: first arg must be an identifier");
            ...
            "min" | "max" if args.len() == 2 => {
                ...
                format!("{{ let (_a, _b) = ({}, {}); if _a {} _b {{ _a }} else {{ _b }} }}", a, b, cmp)
            }
            "to_int" => {
                let a_ty = self.infer_expr_type(&args[0].node);
                ...
                if name == "band" {
                    if let Expr::FnCall { name: bn, args: bn_args } = &args[1].node {
                        if bn == "bnot" && bn_args.len() == 1 {
                            ...
                            return format!(
                                "{{ let (_x, _y) = ({}, {}); Integer::from(&_x - &Integer::from(&_x & &_y)) }}",
                                x, y
                            );
                        }
                    }
            ...
            "shl" if args.len() == 2 => {
                ...
                format!("{{ let (_k, _a) = ({}, Integer::from({})); if _k < 0 || _k > (1i64 << 24) {{ panic!(\"soma:type: shl(): shift count {{}} out of range\", _k) }} _soma_bits_cap(_a << (_k as u32)) }}", b, a)
            }
            "shr" if args.len() == 2 => {
                let a = self.gen_expr_rug(&args[0].node);
                let b = self.gen_expr_rug(&args[1].node);
                format!("{{ let (_a, _k) = (Integer::from({}), Integer::from({})); if _k < 0 {{ panic!(\"soma:type: shr(): shift count {{}} out of range\", _k) }} if _k > u32::MAX {{ Integer::from(if _a < 0 {{ -1 }} else {{ 0 }}) }} else {{ _a >> _k.to_u32().unwrap() }} }}", a, b)
            }
            "bit_len" if args.len() == 1 => {
                let a = self.gen_expr_rug(&args[0].node);
            ...
            "bit_test" if args.len() == 2 => {
                ...
                if let Expr::Ident(name) = &args[0].node {
                    if self.small_int_vars.contains(name) {
                        return format!("Integer::from({{ let (_k, _a): (i64, i64) = ({}, {}); if _k >= 64 {{ if _a < 0 {{ 1i64 }} else {{ 0i64 }} }} else {{ (_a >> _k) & 1i64 }} }})", b, name);
                    }
                    return format!(
            ...
            "bit_set" if args.len() == 2 => {
                ...
                let a = self.gen_expr_rug(&args[0].node);
                let b = format!("_soma_bit_index({})", self.gen_int_to_i64_rug(&args[1].node));
                format!("{{ let (mut _t, _k) = (Integer::from({}), {}); _t.set_bit(_k as u32, true); _soma_bits_cap(_t) }}", a, b)
            }
            "bit_clr" if args.len() == 2 => {
                let a = self.gen_expr_rug(&args[0].node);
                let b = format!("_soma_bit_index({})", self.gen_int_to_i64_rug(&args[1].node));
                format!("{{ let (mut _t, _k) = (Integer::from({}), {}); _t.set_bit(_k as u32, false); _soma_bits_cap(_t) }}", a, b)
            }
            ...
            "bit_next" if args.len() == 2 => {
                ...
                if let Expr::Ident(name) = &args[0].node {
                    if self.small_int_vars.contains(name) {
                        ...
                        return format!("Integer::from(_soma_next_bit({}, {}))", name, b);
                    }
                    return format!(
