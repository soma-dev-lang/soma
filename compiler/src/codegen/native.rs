...
pub fn generate_native_source_with_config(
    ...
    if all_direct_originally {
        // Marker for native_ffi.rs: every handler classifier-picked
        // Direct, so the cell is "pure direct" — no overflow → no
        // panic → no Rug fallback ever needed. native_ffi.rs uses
        // this to set OC=false in the Cargo profile, unlocking
        // unchecked arithmetic speed (~30% on tight i64 loops).
        out.push_str("// SOMA_MODE: ALL_DIRECT\n");
    }
    out.push_str("\n");
