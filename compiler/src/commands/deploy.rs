...
/// A Dockerfile that builds soma FOR the container: copying the host
/// binary shipped a macOS Mach-O into debian and the container died on
/// start. The build stage compiles the released tag from source (needs
/// GMP for BigInt); the runtime stage is slim.
fn dockerfile_for(entry: &str, copy_lines: &str) -> String {
    format!(
        "# syntax=docker/dockerfile:1\n\
         FROM rust:1-bookworm AS build\n\
         RUN apt-get update && apt-get install -y libgmp-dev m4 && rm -rf /var/lib/apt/lists/*\n\
         RUN git clone --depth 1 --branch v{ver} https://github.com/soma-dev-lang/soma /src\n\
         RUN cd /src/compiler && cargo build --release\n\
         \n\
         FROM debian:bookworm-slim\n\
         RUN apt-get update && apt-get install -y ca-certificates libgmp10 && rm -rf /var/lib/apt/lists/*\n\
         COPY --from=build /src/compiler/target/release/soma /usr/local/bin/soma\n\
         {copy}\n\
         WORKDIR /app\n\
         EXPOSE 8080\n\
         CMD [\"soma\", \"serve\", \"{entry}\", \"-p\", \"8080\", \"--host\", \"0.0.0.0\"]\n",
        ver = env!("CARGO_PKG_VERSION"), copy = copy_lines, entry = entry
    )
}
...
fn generate_cloudflare(
    ...
    let dockerfile = dockerfile_for(entry, &copy_lines);
        ...
        if provider.resolve.get(&key).map(|s| s.as_str()) == Some("d1") {
            ...
            let db_id = match output {
                ...
                Err(e) => {
                    ...
                    eprintln!("  the Dockerfile was generated; nothing was deployed");
                    std::process::exit(1);
                }
            };
...
fn generate_fly(
    ...
    match launch {
        ...
        Err(e) => {
            eprintln!("error: fly CLI not found ({}). Install: curl -L https://fly.io/install.sh | sh", e);
            eprintln!("Dockerfile and fly.toml were generated — run `fly deploy` there once the CLI is installed");
            std::process::exit(1);
        }
    }
    ...
    match deploy {
        ...
        Ok(s) => { eprintln!("fly deploy exited with {}", s); std::process::exit(1); }
