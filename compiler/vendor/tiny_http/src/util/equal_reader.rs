    ...
    R: Read,
{
    fn drop(&mut self) {
        ...
        while remaining_to_read > 0 {
            // soma patch: drain in bounded chunks — a declared Content-Length
            // of 10^15 allocated it all at once and aborted the process
            let mut buf = vec![0; remaining_to_read.min(64 * 1024)];
