...
224 builtins. ✗ marks the nondeterministic set (random, now, now_ms, today) — calls to these
...
| `stddev` | `stddev(xs: List) -> Float` | Same as pstdev. |
| `stdev` | `stdev(xs: List) -> Float` | Same as pstdev. |
| `variance` | `variance(xs: List) -> Float` | Population variance (divide by n) — statistics.pvariance. |
