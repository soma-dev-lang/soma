...
| Cost bound | `cost { tokens: N }` holds when every `think()` has a literal `max_tokens` and runs a known number of times — otherwise it is reported as *advisory*, never as proven |
