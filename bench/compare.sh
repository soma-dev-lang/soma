...
py_inner_ms() {
  $PY "bench/py/$1.py" 2>&1 1>/dev/null | grep -oE 'INNER:[0-9]+' | head -1 | cut -d: -f2
}

# Sum of (<N>ms) markers in the soma cell output (the headline + setup checks).
soma_inner_ms() {
  $SOMA run "examples/$1.cell" 2>&1 \
    | grep -oE '\([0-9]+ms\)' \
    | grep -oE '[0-9]+' \
...
run_one() {
  ...
  # Speedup based on inner timing if both are numeric, else wall
  local speedup="—"
  if [[ "$soma_inner" =~ ^[0-9]+$ ]] && [[ "$py_inner" =~ ^[0-9]+$ ]] && [ "$soma_inner" -gt 0 ]; then
